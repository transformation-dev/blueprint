// TODO: Stop using this once this is in its own package. The one in /api/tree is more recent.
...
export async function onRequest({ request, env, params }) {
  ...
  if (idString) {
    ...
    id = ['production', 'preview'].includes(env.CF_ENV) ? env.TEMPORAL_ENTITY.newUniqueId() : env.TEMPORAL_ENTITY.idFromName(crypto.randomUUID())
  }
  ...
  if (response.status >= 400) {
    debug('DURABLE_OBJECT.fetch() to %O failed with status: %O', url, response.status)
