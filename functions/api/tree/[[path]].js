...
export async function onRequest({ request, env, params }) {
  ...
  let id
  if (idString) {
    id = env.TREE.idFromString(idString)
  } else {
    id = ['production', 'preview'].includes(env.CF_ENV) ? env.TREE.newUniqueId() : env.TREE.idFromName(crypto.randomUUID()) // TODO: newUniqueId() fails in `wrangler pages dev` maybe because I'm using old miniflare/wrangler
  }
